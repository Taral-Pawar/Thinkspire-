...
int paperInventory = 100; // Initialize with 100 papers
const int LOW_PAPER_THRESHOLD = 10;
...
bool updatePaperInventory(int printQuantity) {
    if (paperInventory >= printQuantity) {
        paperInventory -= printQuantity;
        Serial.printf("Print job used %d papers. Remaining inventory: %d\n", printQuantity, paperInventory);
        return true;
    } else {
        Serial.printf("Not enough paper! Requested: %d, Available: %d\n", printQuantity, paperInventory);
        return false;
    }
}

// Function to check paper status
String getPaperStatus() {
    if (paperInventory < LOW_PAPER_THRESHOLD) {
        return "low";
    }
    return "ok";
}

// Function to refill paper
void refillPaper(int amount) {
    paperInventory += amount;
    Serial.printf("Paper refilled. Added %d sheets. New inventory: %d\n", amount, paperInventory);
}
...
void setupServer() {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        // Create paper status message
        String paperStatusMsg = "";
        if (paperInventory < LOW_PAPER_THRESHOLD) {
            paperStatusMsg = "<div class='paper-alert'>WARNING: Paper level is low! Only " + String(paperInventory) + " sheets remaining. Please refill paper.</div>";
        }
        ...
        String page = "<!DOCTYPE html><html><head><title>ESP32 File Upload</title>"
                      ...
                      ".paper-status { margin: 20px 0; padding: 15px; border-radius: 8px; background-color: #e8f5e9; display: flex; justify-content: space-between; align-items: center; }"
                      ".paper-status-text { font-size: 1.2rem; font-weight: bold; }"
                      ".refill-btn { background-color: #28a745; color: white; border: none; padding: 10px 20px; cursor: pointer; border-radius: 6px; transition: background 0.3s ease; font-size: 1.1rem; }"
                      ".refill-btn:hover { background-color: #218838; transform: translateY(-2px); }"
                      ".paper-alert { background-color: #f8d7da; color: #721c24; padding: 15px; border-radius: 8px; margin: 15px 0; font-size: 1.2rem; font-weight: bold; border: 2px solid #f5c6cb; }"
                      ...
                      "<span class='paper-status-text'>Paper Inventory: " + String(paperInventory) + " sheets</span>"
                      "<button onclick='refillPaper()' class='refill-btn'>Refill Paper</button>"
                      "</div>"
                      "<div class='section'>"
                      ...
                      "  const refillAmount = prompt('Enter number of sheets to add:', '100');"
                      "  if (refillAmount !== null && !isNaN(refillAmount) && parseInt(refillAmount) > 0) {"
                      "    fetch('/refill-paper?amount=' + encodeURIComponent(refillAmount), {"
                      "      method: 'GET'"
                      "    })"
                      "    .then(response => {"
                      "      if (response.ok) {"
                      "        alert('Paper inventory refilled successfully!');"
                      "        location.reload();"
                      "      } else {"
                      "        alert('Error refilling paper inventory');"
                      "      }"
                      "    })"
                      "    .catch(error => {"
                      "      console.error('Error:', error);"
                      "      alert('An error occurred while refilling paper inventory.');"
                      "    });"
                      "  }"
                      "}"
    ...
    // Paper Refill Handler
    server.on("/refill-paper", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasParam("amount")) {
            int amount = request->getParam("amount")->value().toInt();
            if (amount > 0) {
                refillPaper(amount);
                request->send(200, "text/plain", "Paper refilled successfully");
            } else {
                request->send(400, "text/plain", "Invalid refill amount");
            }
        } else {
            request->send(400, "text/plain", "Amount parameter missing");
        }
    });

    // Get Paper Status API
    server.on("/paper-status", HTTP_GET, [](AsyncWebServerRequest *request) {
        DynamicJsonDocument doc(128);
        doc["inventory"] = paperInventory;
        doc["status"] = getPaperStatus();
    ...
    server.on("/payment", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        String paymentPage = "<!DOCTYPE html><html><head><title>Payment - Team Thinkspire</title>"
                            ...
                            ".paper-warning { background-color: #f8d7da; color: #721c24; padding: 15px; border-radius: 8px; margin: 15px 0; font-size: 1.2rem; font-weight: bold; border: 2px solid #f5c6cb; display: none; }"
                            "@media (max-width: 768px) {"
                            "  .container { padding: 30px 20px; width: 95%; }"
                            "  h1 { font-size: 2rem; }"
                            "  h2 { font-size: 1.5rem; }"
                            ...
                            "<h1>TEAM THINKSPIRE</h1>"
                            "<h2>Payment</h2>"
                            "<div id='paperWarning' class='paper-warning'></div>"
                            "<div class='section'>"
                            ...
                            "    const paperWarning = document.getElementById('paperWarning');"
                            "    if (data.inventory < " + String(LOW_PAPER_THRESHOLD) + ") {"
                            "      paperWarning.style.display = 'block';"
                            "      paperWarning.innerHTML = 'WARNING: Paper level is low! Only ' + data.inventory + ' sheets remaining.';"
                            "    }"
                            "    if (data.inventory < printQuantity) {"
                            "      paperWarning.style.display = 'block';"
                            "      paperWarning.innerHTML = 'ERROR: Not enough paper for this job! Available: ' + data.inventory + ', Required: ' + printQuantity;"
                            "      document.querySelector('.print-btn').disabled = true;"
                            "      document.querySelector('.print-btn').style.backgroundColor = '#6c757d';"
                            "    }"
                            "  })"
                            "  .catch(error => {"
                            ...
                            "  checkPaperStatus();"
                            "  alert('Please make payment to the UPI ID shown to continue');"
                            "});"
                            "</script>"
                            "</body></html>";
    ...
    server.on("/print", HTTP_POST, [](AsyncWebServerRequest *request) {
        ...
        if (updatePaperInventory(quantity)) {
            Serial.printf("Print job received: %d copies on %s %s paper\n", quantity, paperSize.c_str(), paperType.c_str());
            responseDoc["success"] = true;
            responseDoc["message"] = "Print job started successfully";
        } else {
            responseDoc["success"] = false;
            responseDoc["message"] = "Not enough paper for this job";
        }
...
void setup() {
    ...
    // Initialize paper inventory
    paperInventory = 100;
    Serial.println("Paper inventory initialized to 100 sheets");
